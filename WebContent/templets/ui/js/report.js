...
function returnWin(isref,par){
	if(par == null ||par == "undefined" || par == undefined){
		par = window.parent;
	}
	par.document.getElementById("feedBackTxt").value=document.getElementById("BackTxt").value;
}
...
function showWin(tit, uri, conwin, methodnm, par, wh, hh, winpos) {
	window.ymPrompt = openSubWin('myIf', tit, uri, wh, hh, false, true,
			function(tp) {
				winHandler(tp, conwin, methodnm);
			}, true);
}
...
function winHandler(tp, conwin, methodnm) {
	...
	if (tp == "ref") {
		if (conwin == "report") {
			var wn = window.parent.document.getElementById('leftNavgframe')
					|| window.parent.parent.document
							.getElementById('leftNavgframe');
			var cwin = wn.contentWindow;
			var eventName = methodnm.substring(0, methodnm.length - 2);
			eval("if(cwin." + eventName + "){cwin." + methodnm + "}else{"
					+ methodnm + "}");
		} else {
			eval(methodnm);
		}
	...
	create : function(str, sep, name, vals, isSeq) {
		...
		for (var i = 0; i < strArr.length; i++) {
			...
			if (strArr[i] != null && strArr[i].length > 0) {
				...
				if (isSeq) {
					html += "<input type=\"text\" class=\"editor\" name=\"seq_"
							+ name
							+ "\" id=\"seq_"
							+ name
							+ "_"
							+ code
							+ "\" value=\"\" style=\"width:50px;\" title=\"����˳��\"/>";
				}
				html += "<input type=\"checkbox\" " + chk + " name=\"" + name
						+ "\" id=\"" + name + "_" + code + "\" value=\"" + code
						+ "\"><label for=\"" + name + "_" + code + "\">" + txt
						+ "</label>";

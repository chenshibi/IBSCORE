...
<#--jianxue.zhang-->
<@CommonQueryMacro.page title="系统参数设置审核">
<table width="100%" align="left">
<tr>
<td>
<@CommonQueryMacro.CommonQuery id="SysParamsCheck" init="false" submitMode="current">
	<table width="100%">
		<tr>
   			<td valign="top" colspan="2">
   			<@CommonQueryMacro.Interface id="interface" label="请输入查询条件" fieldStr="id" />
        	</td>
        </tr>
		<tr>
   			<td colspan="2">
  		...
  		<tr>
      		<td colspan="2">
          		<@CommonQueryMacro.DataTable id ="datatable1" paginationbar="btDo" fieldStr="select,id[220],intInsId[200],adtRcdPk[200],updTransCd,intOperId,crtDt" width="100%"  readonly="true"/><br />
        	</td>
        </tr>
...
function initCallGetter_post() {
	 ...
	 if(intInsId!=null && intInsId.length>0){
	 	SysParamsCheck_interface_dataset.setValue("intInsId", intInsId);
	 	SysParamsCheck_dataset.flushData(SysParamsCheck_dataset.pageIndex);
	 }
 }

//定位一条记录
function locate(id) {
	var record = SysParamsCheck_dataset.find(["id"],[id]);
	if (record) {
		SysParamsCheck_dataset.setRecord(record);
	}
}
...
function btDo_onClickCheck(button){
	var tlrno = "${v_tlrno}";
	var record = SysParamsCheck_dataset.firstUnit;
	var chk=0;

...
<@CommonQueryMacro.page title="提交企业征信报告查询请求(紧急)">
<@CommonQueryMacro.CommonQuery id="UrgentPbocQueryCorpMake" init="false" submitMode="current">
...
<script language="javascript">
    ...
    function initCallGetter_post(dataset) {
      UrgentPbocQueryCorpMake_dataset.setValue("serviceCode","FW_QYXYBG_0043");
      UrgentPbocQueryCorpMake_dataset.setValue("queryLevel","2");
    }

  	function btSearch_onClickCheck(button){
      var entName=UrgentPbocQueryCorpMake_dataset.getValue("entName");
      var entCertType=UrgentPbocQueryCorpMake_dataset.getValue("entCertType");
      var entCertNum=UrgentPbocQueryCorpMake_dataset.getValue("entCertNum");
      var queryReason=UrgentPbocQueryCorpMake_dataset.getValue("queryReason");
      var serviceCode=UrgentPbocQueryCorpMake_dataset.getValue("serviceCode");
      if(entName==""){
         alert("企业名称不可为空");
         return false;
      }
      ...
      if(serviceCode==""){
	   ...
	   UrgentPbocQueryCorpMake_dataset.setValue("nonWorkhourFilepath",nonWorkhourFilepath);

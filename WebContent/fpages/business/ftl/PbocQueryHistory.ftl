...
<@CommonQueryMacro.CommonQuery id="PbocQueryHistory" init="false" submitMode="current" navigate="false">
	<table align="left" width="100%">
     ...
     <tr>
      		<td>
      			<@CommonQueryMacro.DataTable id ="datatable1" fieldStr="opr[300],entName[110],entCertType,entCertNum,queryReason,serviceCode[120],respId[200],respCode,respMsg,queryDate,createUser,createTime,checkUser,checkTime,sendTime,respTime,status,certAuthStatus" readonly="true" width="100%"/>
      		</td>
     </tr>
</table>
...
<script language="JavaScript">
	...
	function datatable1_opr_onRefresh(cell, value, record){
		...
		if(record) {//当存在记录时
			...
			//}
			if("66661706"==funcid6){
				htmlStr=htmlStr+"&nbsp&nbsp&nbsp&nbsp<a href=\"JavaScript:queryDownload('"+entCertNum+"')\">查询授权书下载</a>"
			}
			if(status=="04"){
	...
	<#--授权书下载-->
	function queryDownload(entCertNum){
		dwr.engine.setAsync(false)
		var flag=PrivAction.test7(entCertNum)
		dwr.engine.setAsync(false)
		if(flag=="1"){
			var paramMap = new Map()
	 		paramMap.put("entCertNum",entCertNum)
			window.location.href="${contextPath}/DownloadLoaAction2.do?entCertNum="+entCertNum}
			else{
	...
	function queryDownload(entCertNum){
		dwr.engine.setAsync(false)
		var flag=PrivAction.test5(entCertNum)
		dwr.engine.setAsync(false)
		if(flag=="1"){
			var paramMap = new Map()
	 		paramMap.put("entCertNum",entCertNum)
			window.location.href="${contextPath}/DownloadLoaAction.do?entCertNum="+entCertNum}
			else{
				alert("The file does not exists！")
			}
	}	
	...
	function reQuery(entName,entCertType,entCertNum,queryReason,serviceCode,queryLevel){
	  ...
	  if(isCompanyExpire == "0"){
			    	 ...
			    	 showWin("查询授权书上传", "/fpages/business/ftl/TCorpPermitUploadN2.ftl?corpCustCompanyname=" + entName + "&corpCustLoancard=" + entCertNum,300,300,window);
			    	 return false;
				 }

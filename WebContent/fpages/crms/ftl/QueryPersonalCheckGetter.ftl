...
<@CommonQueryMacro.page title="个人征信查询审核">
<table width="100%" align="left">
<tr>
<td>
<@CommonQueryMacro.CommonQuery id="QueryPersonalCheckGetter" init="false" submitMode="selected">
	<table width="100%">
		<tr>
		...
		<tr>
  		...
  		<tr>
      		<td colspan="2">
          		<@CommonQueryMacro.DataTable id ="datatable1" paginationbar="btBatchApprove,-,btBatchReject" fieldStr="select,createUser,name,idType,idNum,queryReason,serviceCode,queryDate,status" width="100%"  readonly="true"/><br />
        	</td>
        </tr>
...
function btBatchApprove_onClickCheck() {
    if (!isRecordSelected(QueryPersonalCheckGetter_dataset)) {
        alert("请选择一条记录！");
        return false;
    }
    return true;
...
function flushData() {
    QueryPersonalCheckGetter_dataset.flushData(1);
}
    ...
    if (button.returnParam && button.returnParam.uuid) {
           ...
           var t1=t.replace("[","");
            ...
            for(var i=0;i<len;i++){
                ...
                OpenWindowWithPost("${contextPath}/CrmsPersonalReportServlet", null, "_blank", params);
            }
    } else {
        alert("查询失败！");
    }

...
public class GenFrontPageForSingleTable {
    ...
    public static String FTL_HEAD_LINE_FD_DETAIL = "                        <@CommonQueryMacro.DataTable id=\"datatable1\" fieldStr=\"ftloperation${fieldstr}\"  width=\"100%\" hasFrame=\"true\"/>\n";
    public static String FTL_HEAD_LINE_FD = "                        <@CommonQueryMacro.DataTable id=\"datatable1\" fieldStr=\"${fieldstr}\"  width=\"100%\" hasFrame=\"true\"/>\n";
        ...
        for (int i = 0; i < tableTable.getRowCount(); i++) {
            ...
            if (StringUtils.equalsIgnoreCase(inWhere, "true")) {
                if (StringUtils.equalsIgnoreCase(whereType, "like")) {
                    ...
                    String cond1 = " and " + fieldName + " >= ? ";
                    String cond2 = " and " + fieldName + " <= ? ";
    ...
    public static void genXmlFile(String title, String module, DefaultTableModel tableTable,
                                  String tableName, boolean genDetail, String pageType) throws Exception {
        ...
        if (genDetail || pageType.equalsIgnoreCase("Query") == false) {
        ...
        for (int i = 0; i < tableTable.getRowCount(); i++) {
            ...
            if (StringUtils.equalsIgnoreCase(inWhere, "true")) {
                if (StringUtils.equalsIgnoreCase(editType, "select")) {
                    ...
                    out.write(("        <Option id=\"query" + getJavaName(tableName, fieldName) + "\" require=\"false\" " + " desc=\""
                            + fieldDesc + "\" datatype=\"select\" index=\"" + index + "\" " + translated + "/>\n")
                            .getBytes("UTF-8"));
                    index++;
                } else {
                    if (StringUtils.equalsIgnoreCase(whereType, "between")) {
                        out.write(("        <TextBox id=\"query" + getJavaName(tableName, fieldName) + "Start\" require=\"false\" " + " desc=\""
                                + fieldDesc + "\" datatype=\"" + dataType + "\" index=\"" + index + "\" " + " size=\""
                                + len + "\"" + "/>\n").getBytes("UTF-8"));
                        index++;
                        out.write(("        <TextBox id=\"query" + getJavaName(tableName, fieldName) + "End\" require=\"false\" " + " desc=\""
                                + fieldDesc + "\" datatype=\"" + dataType + "\" index=\"" + index + "\" " + " size=\""
                                + len + "\"" + "/>\n").getBytes("UTF-8"));
                        index++;
                    } else {
    ...
    public static String getDetailGetterClassName(String pageType, String table) {
        StringBuffer sb = new StringBuffer();
        sb.append(ColumnItem.getClassName(table));
        sb.append(pageType + "DetailGetter");
        return sb.toString();
    }
    ...
    public static void genFtlFile(String title, String module, DefaultTableModel tableTable,
        ...
        out.write(FTL_HEAD_LINE_18.replace("${title}", title).replace("${module}", module)
                .replace("${getterclassname}", getGetterClassName(pageType, tableName)).getBytes("UTF-8"));
        if (genDetail) {
            out.write(FTL_HEAD_LINE_FD_DETAIL.replace("${fieldstr}", fieldStr).replace("${module}", module)
                    .replace("${getterclassname}", getGetterClassName(pageType, tableName)).getBytes("UTF-8"));
        } else {
            out.write(FTL_HEAD_LINE_FD.replace("${fieldstr}", fieldStr.substring(1)).replace("${module}", module)
                    .replace("${getterclassname}", getGetterClassName(pageType, tableName)).getBytes("UTF-8"));
        }
        ...
        if (genDetail || pageType.equalsIgnoreCase("Query") == false) {
             ...
             * if(record!=null){ var st = record.getValue("st"); var id1 =
             * record.getValue("paramgroupId"); var id2 =
             * record.getValue("paramId"); var id = id2+"#"+id1;
             *
             *
             *
             * cell.innerHTML = "<a
             * href=\"Javascript:showDetail('"+id1+"','"+id2+"','"+st+"')\">"+id1+"</a>";
             *
             * } else { cell.innerHTML = "" }
             *
             *
            ...
            out.write(
                    ("        loadPageWindows(\"partWin\", \"查看明细\",\"/fpages/${module}/ftl/${detailgetterclassname}.ftl\", paramMap, \"winZone\");\n")
                            .replace("${module}", module)
                            .replace("${detailgetterclassname}", getDetailGetterClassName(pageType, tableName)).getBytes("UTF-8"));
    ...
    public static void genFrontPage(String title, String module, DefaultTableModel tableTable,
                                    String tableName, String baseWhere, boolean genDetail, String pageType) throws Exception {

        condMap.clear();
        keyList.clear();
        genXmlFile(title, module, tableTable, tableName, genDetail, pageType);
        genGetterClass(title, module, tableTable, tableName, baseWhere, pageType);
        genFtlFile(title, module, tableTable, tableName, genDetail, pageType);
    }

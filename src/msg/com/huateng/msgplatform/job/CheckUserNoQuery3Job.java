...
/**
 * 30天没有任何查询操作的用户锁定，域值可以配置，发邮件给锁定用户
 * @author chensibi
 *
 */

public class CheckUserNoQuery3Job implements org.quartz.StatefulJob{
	 private Logger htlog = Logger.getLogger(CheckUserNoQuery3Job.class);
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		//获取系统参数
		 SysParamsDAO dao=DAOUtils.getSysParamsDAO();
         List list=dao.findByProperty("paramName", "没有任何查询笔数");
         SysParams param=(SysParams)list.get(0);
         String month=param.getParamVal();
         
         Date endDate = new Date();//当前日期
         //根据终止日计算起始日
         Date startDate = DateUtils.getStartDateByDays(endDate, Integer.valueOf(month));//判断起始日
         ...
         buffer.append(a);
         buffer.append("'");
         buffer.append(" group by CREATE_USER having count(CREATE_USER) > 0 '");
         try {
	...
	private void findTrlInfo(){
		StringBuffer str=new StringBuffer();
		str.append("select * from tlr_info where IS_LOCK = '1'");
		try {
			List sql = DAOUtils.getHQLDAO().queryBySQL2List(str.toString());
			for(int i=0;i<sql.size();i++){
				TlrInfo info =(TlrInfo)sql.get(i);
				String userName=info.getTlrno();
				sendMails(userName);
			}
		} catch (CommonException e) {
			e.printStackTrace();

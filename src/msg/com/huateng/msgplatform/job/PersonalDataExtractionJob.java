...
public class PersonalDataExtractionJob implements org.quartz.StatefulJob {
    @SuppressWarnings("unused")
	private Logger htlog = Logger.getLogger(PersonalDataExtractionJob.class);
        ...
        try {
            ...
            StringBuffer sb=new StringBuffer();
   		    sb.append("select B.resp_Id from  Cust_Pboc_Per_Query B ")
   			 .append("  where B.query_Date BETWEEN ").append("'").append(firstMonthDay).append("'")
   			 .append("  and '").append(lastMonthDay).append("' ");
			String inSql=sb.toString();
            LinkedHashMap<String, String> sqlMap = WriteDbToExcelUtil.getPerGen2TableMap(inSql);
            Iterator<Entry<String, String>> iterator = sqlMap.entrySet().iterator();
            String path = ReportUtils.getSysParamsValue("DATA", "GENPER") + File.separator + firstMonthDay.substring(0, 6) + File.separator;
            ...
            htlog.info("==========="+"start time :"+start+"(ms)");
             while (iterator.hasNext()) { 
            	  ...
            	  String perGen2Sql = SqlConstantUtils.getPerGen2Sql(inSql, entry.getKey().trim());

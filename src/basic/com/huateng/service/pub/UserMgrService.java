        ...
        try {
            ...
            if (tlrList.isEmpty()) {
                ...
                List list = hqldao.queryByQL2List(
                        "from TlrBctlRel where  tlrNo='" + userLoginId + "'");
                if (list.size() <= 0) {
...
//                tlrInfo.setBrcode(bctl.getBrcode());
    ...
    /**
     * 获取用户会话信息，交行优化版本 原因：交行需要实现切换操作员角色
     *
     * @param String
     *            userID 柜员编号
     *
    ...
    public UserSessionInfo getUserSessionInfo(TlrInfo tlrinfo) throws CommonException {
        ...
        try {
            ...
            // ExceptionUtil.throwCommonException("操作员默认角色编号配置错误",
...
//            sessionInfo.setBrName(bctl.getBrname());
    ...
    /**
     * Description: 用户登录时，校验用户密码，并且获取用户角色权限信息
     *
     * @param
    ...
    /**
     * Description: 切换角色，重启session中权限信息
     *
     * @param curRoleId
     *            当前角色号
     * @return void
    ...
    public void getUserFunctionsByMenuType_favt(String tlrno, String menuCode, List<FunctionInfo> funcInfoList) {

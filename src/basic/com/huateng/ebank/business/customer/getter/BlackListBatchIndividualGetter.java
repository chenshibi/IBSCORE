 ...
 *
 */
public class BlackListBatchIndividualGetter extends BaseGetter {
	
	private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(BlackListBatchIndividualGetter.class);
	...
	protected PageQueryResult getData() throws Exception {
		...
		String custIdi = "";*/
   	 	...
   	 	ArrayList<String> listCustId = new ArrayList<String>();
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custId"))){
    		String custId  = (String) getCommQueryServletRequest().getParameterMap().get("custId");
    		listCustId.add(custId);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIda"))){
    		String custIda  = (String) getCommQueryServletRequest().getParameterMap().get("custIda");
    		listCustId.add(custIda);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdb"))){
    		String custIdb  = (String) getCommQueryServletRequest().getParameterMap().get("custIdb");
    		listCustId.add(custIdb);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdc"))){
    		String custIdc  = (String) getCommQueryServletRequest().getParameterMap().get("custIdc");
    		listCustId.add(custIdc);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdd"))){
    		String custIdd  = (String) getCommQueryServletRequest().getParameterMap().get("custIdd");
    		listCustId.add(custIdd);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIde"))){
    		String custIde  = (String) getCommQueryServletRequest().getParameterMap().get("custIde");
    		listCustId.add(custIde);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdf"))){
    		String custIdf  = (String) getCommQueryServletRequest().getParameterMap().get("custIdf");
    		listCustId.add(custIdf);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdg"))){
    		String custIdg  = (String) getCommQueryServletRequest().getParameterMap().get("custIdg");
    		listCustId.add(custIdg);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdh"))){
    		String custIdh  = (String) getCommQueryServletRequest().getParameterMap().get("custIdh");
    		listCustId.add(custIdh);
    	}
    	if(!"".equals((String) getCommQueryServletRequest().getParameterMap().get("custIdi"))){
    		String custIdi  = (String) getCommQueryServletRequest().getParameterMap().get("custIdi");
    		listCustId.add(custIdi);
    	}
    	 
    	String custId = listCustId.get(0);
		for(int i = 1 ;i < listCustId.size();i++){
			custId = custId + "','" + listCustId.get(i);//用','分隔拼成长字符串:"2016062100002952336883,2016062100002952309411"       
		}
		...
		*/
        ...
        hql.append("and t.CUST_ID in ('"+custId+"')");			
        }

 ...
 */
public class batchTlrUpdateInfoQueryGetter extends BaseGetter {
    ...
    private PageQueryResult getData() throws CommonException {
    	...
    	//换oracle数据库，调整日期格式  20191125-qx
    	 String startDate1 = startDate.substring(0, 4)+"-"+startDate.substring(4, 6)+"-"+startDate.substring(6, 8);
    	 String endDate1 = endDate.substring(0,4)+"-"+endDate.substring(4,6)+"-"+endDate.substring(6,8);
    	
    	 
    	
         PageQueryResult pageQueryResult = new PageQueryResult();
         ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
         List<TlrUpdateInfoBatch> list = new ArrayList(); 
         StringBuffer hql = new StringBuffer("select count(batchId) as countId,inputTime,createUser,batchId from TlrUpdateInfoBatch where 1=1 ");
         ...
         List listTlrUpdateInfoBatchBean=rootdao.queryByCondition(hql.toString());
         if(null != listTlrUpdateInfoBatchBean && listTlrUpdateInfoBatchBean.size()>0){
        	 for (int i = 0; i < listTlrUpdateInfoBatchBean.size(); i++) {
        		 TlrUpdateInfoBatch tlrUpdateInfoBatch=new TlrUpdateInfoBatch();
        		 Object[] obj=(Object[]) listTlrUpdateInfoBatchBean.get(i);
        		 tlrUpdateInfoBatch.setCountid((Long) obj[0]);
        		 tlrUpdateInfoBatch.setInputTime((Date) obj[1]);
        		 tlrUpdateInfoBatch.setCreateUser((String) obj[2]);
        		 tlrUpdateInfoBatch.setBatchId((Integer) obj[3]);

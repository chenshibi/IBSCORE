 ...
 *
 */
public class TcorpDetailOthersDraftQueryGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
        ...
        StringBuffer hql = new StringBuffer("select ia from TCorpDetailOthers ia where ia.itemName='银行承兑汇票' and ia.status='未结清' and ia.fiveLevel <>'关注' and ia.fiveLevel <> '' and ia.rptKey='"+rptKey+"'");
       ...
       for (int i = 0; i < listTcorpDetailOthers.size(); i++) {
		{
        	...
        	tcorpDetailOthers.setDepositRatio(listTcorpDetailOthers.get(i).getDepositRatio());
        	tcorpDetailOthers.setFiveLevel(listTcorpDetailOthers.get(i).getFiveLevel());
        	tcorpDetailOthers.setAssured(listTcorpDetailOthers.get(i).getAssured());
        	tcorpDetailOthers.setAdvanced(listTcorpDetailOthers.get(i).getAdvanced());

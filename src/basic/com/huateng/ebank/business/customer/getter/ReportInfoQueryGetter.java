...
public class ReportInfoQueryGetter extends BaseGetter {
         ...
         ROOTDAO rootdao = ROOTDAOUtils.getROOTDAO();
          ...
          StringBuilder hq = new StringBuilder(" SELECT model FROM IbsEntCreditSum model WHERE 1 = 1 ");
  		  ...
  		  if(null!=queryListByCorp && queryListByCorp.size()>0) {
  			 ...
  			 perCreditSum.setYqje(String.valueOf(queryListByCorp.get(0).getDqgzlblfzbs()));
  		  ...
  		  Double pDsr=0.00;
  		  Double pWc=0.00;
  		  IbsPerCreditSum ibsPerCreditSum2=null;
	             	 ...
	             	 List<IbsPerCreditSum> queryByCondition  = rootdao.queryByCondition(hql.toString());
 	             	 if(queryByCondition!=null && queryByCondition.size()>0) {
	             		 for(int j=0;j<queryByCondition.size();j++) {
	             			 ibsPerCreditSum2 = queryByCondition.get(j);
	             			 Double pTue2=Airth.StringToDouble(ibsPerCreditSum2.getTue());
	             			 Double pDsr2=Airth.StringToDouble(ibsPerCreditSum2.getDsr());
	             			 Double pWc2=Airth.StringToDouble(ibsPerCreditSum2.getWc());
	             			...
	             			pTue=Airth.add(pTue,pTue2);
	             			pDsr=Airth.add(pDsr,pDsr2);
	             			pWc=Airth.add(pWc,pWc2);
	             		 }
	             	 }
	             	
	             	 queryListByPerList.addAll(queryByCondition);
	        	}
	        	 totalTue=Airth.add(tue, pTue);
	        	 totalDsr=Airth.add(dsr, pDsr);
	        	 totalWc=Airth.add(wc, pWc);
	        	 totalTue2=Airth.DoubleToStr(totalTue);
	        	 totalDsr2=Airth.DoubleToStr(totalDsr);
	        	 totalWc2=Airth.DoubleToStr(totalWc);
	     	}
	      }
	     IbsPerCreditSum  newIbsPerCreditSum=new  IbsPerCreditSum();
	     if(queryListByPerList!=null&& queryListByPerList.size()>0) {
	    	  newIbsPerCreditSum.setDbrxm("Tur总金额:"+totalTue2);
	    	  newIbsPerCreditSum.setCxrq("Dsr总金额:"+totalDsr2);
	    	  newIbsPerCreditSum.setSfbh("Wc总金额:"+totalWc2);
	    	  queryListByPerList.add(newIbsPerCreditSum);
	      }
        int pageIndex = getResult().getPage().getCurrentPage();
        int pageSize = getResult().getPage().getEveryPage();
        int maxIndex = pageIndex * pageSize;
        /* 对最后一页的处理 */
        if (maxIndex > queryListByPerList.size()) {
            maxIndex = queryListByPerList.size();
        }
        List result = new ArrayList();
        result = queryListByPerList.subList((pageIndex - 1) * pageSize, maxIndex);
        pageQueryResult.setTotalCount(queryListByPerList.size());

 ...
 *
 */
public class TcorpSocialPayRecordFundsQueryGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
        ...
        StringBuffer hql = new StringBuffer("select ia from TCorpSocialPayRecord ia where ia.itemName='住房公积金缴费记录' and ia.rptKey='"+rptKey+"'");
        List<TCorpSocialPayRecord> listTcorpSocialPayRecord = rootdao.queryByCondition(hql.toString());
    	   for (int i = 0; i < listTcorpSocialPayRecord.size(); i++) {
        	...
        	tcorpSocialPayRecord.setFirstPayYm(listTcorpSocialPayRecord.get(i).getFirstPayYm());
        	tcorpSocialPayRecord.setEmployeeNumber(listTcorpSocialPayRecord.get(i).getEmployeeNumber());
        	tcorpSocialPayRecord.setPayAmt(listTcorpSocialPayRecord.get(i).getPayAmt());
        	tcorpSocialPayRecord.setLastPayDate(listTcorpSocialPayRecord.get(i).getLastPayDate());
        	tcorpSocialPayRecord.setLastPayYm(listTcorpSocialPayRecord.get(i).getLastPayYm());

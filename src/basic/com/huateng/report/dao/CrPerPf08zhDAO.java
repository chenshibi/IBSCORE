...
@Repository
public class CrPerPf08zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf08zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf08zh pojo) {
    ...
    public void save(CrPerPf08zh pojo) {
    ...
    public void delete(CrPerPf08zh pojo) {
    ...
    public CrPerPf08zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf08zh)this.getHibernateTemplate().get(CrPerPf08zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf08zh> findByProperties(CrPerPf08zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf08zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf08zd01())){
                queryString.append(" and model.pf08zd01 = ? ");
                binds.add(pojo.getPf08zd01());
            }
            if(isNotBlankOrNull(pojo.getPf08zq01())){
                queryString.append(" and model.pf08zq01 = ? ");
                binds.add(pojo.getPf08zq01());
            }
            if(isNotBlankOrNull(pojo.getPf08zr01())){
                queryString.append(" and model.pf08zr01 = ? ");
                binds.add(pojo.getPf08zr01());
            }
            ...
            return (List<CrPerPf08zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf08zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf08zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

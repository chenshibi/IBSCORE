...
@Repository
public class CrPerPahDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPahDAO.class);
    ...
    public void update(CrPerPah pojo) {
    ...
    public void save(CrPerPah pojo) {
    ...
    public void delete(CrPerPah pojo) {
    ...
    public CrPerPah findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPah)this.getHibernateTemplate().get(CrPerPah.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPah> findByProperties(CrPerPah pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPah as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPf08aq01())){
                queryString.append(" and model.pf08aq01 = ? ");
                binds.add(pojo.getPf08aq01());
            }
            if(isNotBlankOrNull(pojo.getPf08aq02())){
                queryString.append(" and model.pf08aq02 = ? ");
                binds.add(pojo.getPf08aq02());
            }
            if(isNotBlankOrNull(pojo.getPf08ar01())){
                queryString.append(" and model.pf08ar01 = ? ");
                binds.add(pojo.getPf08ar01());
            }
            if(isNotBlankOrNull(pojo.getPf08ar02())){
                queryString.append(" and model.pf08ar02 = ? ");
                binds.add(pojo.getPf08ar02());
            }
            if(isNotBlankOrNull(pojo.getPf08zs01())){
                queryString.append(" and model.pf08zs01 = ? ");
                binds.add(pojo.getPf08zs01());
            }
            ...
            return (List<CrPerPah>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPah> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPah>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

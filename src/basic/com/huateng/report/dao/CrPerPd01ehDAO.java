...
@Repository
public class CrPerPd01ehDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPd01ehDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPd01eh pojo) {
    ...
    public void save(CrPerPd01eh pojo) {
    ...
    public void delete(CrPerPd01eh pojo) {
    ...
    public CrPerPd01eh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPd01eh)this.getHibernateTemplate().get(CrPerPd01eh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd01eh> findByProperties(CrPerPd01eh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPd01eh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPd01er03())){
                queryString.append(" and model.pd01er03 = ? ");
                binds.add(pojo.getPd01er03());
            }
            if(isNotBlankOrNull(pojo.getPd01ed01())){
                queryString.append(" and model.pd01ed01 = ? ");
                binds.add(pojo.getPd01ed01());
            }
            if(isNotBlankOrNull(pojo.getPd01ej01())){
                queryString.append(" and model.pd01ej01 = ? ");
                binds.add(pojo.getPd01ej01());
            }
            ...
            return (List<CrPerPd01eh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd01eh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPd01eh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

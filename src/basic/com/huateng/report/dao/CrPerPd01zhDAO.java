...
@Repository
public class CrPerPd01zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPd01zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPd01zh pojo) {
    ...
    public void save(CrPerPd01zh pojo) {
    ...
    public void delete(CrPerPd01zh pojo) {
    ...
    public CrPerPd01zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPd01zh)this.getHibernateTemplate().get(CrPerPd01zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd01zh> findByProperties(CrPerPd01zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPd01zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPd01zd01())){
                queryString.append(" and model.pd01zd01 = ? ");
                binds.add(pojo.getPd01zd01());
            }
            if(isNotBlankOrNull(pojo.getPd01zq01())){
                queryString.append(" and model.pd01zq01 = ? ");
                binds.add(pojo.getPd01zq01());
            }
            if(isNotBlankOrNull(pojo.getPd01zr01())){
                queryString.append(" and model.pd01zr01 = ? ");
                binds.add(pojo.getPd01zr01());
            }
            ...
            return (List<CrPerPd01zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd01zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPd01zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

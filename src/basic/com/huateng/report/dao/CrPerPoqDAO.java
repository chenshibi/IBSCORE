...
@Repository
public class CrPerPoqDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPoqDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPoq pojo) {
    ...
    public void save(CrPerPoq pojo) {
    ...
    public void delete(CrPerPoq pojo) {
    ...
    public CrPerPoq findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPoq)this.getHibernateTemplate().get(CrPerPoq.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPoq> findByProperties(CrPerPoq pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPoq as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPh010r01())){
                queryString.append(" and model.ph010r01 = ? ");
                binds.add(pojo.getPh010r01());
            }
            if(isNotBlankOrNull(pojo.getPh010d01())){
                queryString.append(" and model.ph010d01 = ? ");
                binds.add(pojo.getPh010d01());
            }
            if(isNotBlankOrNull(pojo.getPh010q02())){
                queryString.append(" and model.ph010q02 = ? ");
                binds.add(pojo.getPh010q02());
            }
            if(isNotBlankOrNull(pojo.getPh010q03())){
                queryString.append(" and model.ph010q03 = ? ");
                binds.add(pojo.getPh010q03());
            }
            ...
            return (List<CrPerPoq>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPoq> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPoq>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

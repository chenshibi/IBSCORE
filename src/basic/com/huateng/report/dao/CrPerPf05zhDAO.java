...
@Repository
public class CrPerPf05zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf05zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf05zh pojo) {
    ...
    public void save(CrPerPf05zh pojo) {
    ...
    public void delete(CrPerPf05zh pojo) {
    ...
    public CrPerPf05zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf05zh)this.getHibernateTemplate().get(CrPerPf05zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf05zh> findByProperties(CrPerPf05zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf05zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf05zd01())){
                queryString.append(" and model.pf05zd01 = ? ");
                binds.add(pojo.getPf05zd01());
            }
            if(isNotBlankOrNull(pojo.getPf05zq01())){
                queryString.append(" and model.pf05zq01 = ? ");
                binds.add(pojo.getPf05zq01());
            }
            if(isNotBlankOrNull(pojo.getPf05zr01())){
                queryString.append(" and model.pf05zr01 = ? ");
                binds.add(pojo.getPf05zr01());
            }
            ...
            return (List<CrPerPf05zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf05zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf05zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

...
@Repository
public class CrPerPd03zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPd03zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPd03zh pojo) {
    ...
    public void save(CrPerPd03zh pojo) {
    ...
    public void delete(CrPerPd03zh pojo) {
    ...
    public CrPerPd03zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPd03zh)this.getHibernateTemplate().get(CrPerPd03zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd03zh> findByProperties(CrPerPd03zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPd03zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPd03zd01())){
                queryString.append(" and model.pd03zd01 = ? ");
                binds.add(pojo.getPd03zd01());
            }
            if(isNotBlankOrNull(pojo.getPd03zq01())){
                queryString.append(" and model.pd03zq01 = ? ");
                binds.add(pojo.getPd03zq01());
            }
            if(isNotBlankOrNull(pojo.getPd03zr01())){
                queryString.append(" and model.pd03zr01 = ? ");
                binds.add(pojo.getPd03zr01());
            }
            ...
            return (List<CrPerPd03zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPd03zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPd03zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

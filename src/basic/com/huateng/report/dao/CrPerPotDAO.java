...
@Repository
public class CrPerPotDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPotDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPot pojo) {
    ...
    public void save(CrPerPot pojo) {
    ...
    public void delete(CrPerPot pojo) {
    ...
    public CrPerPot findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPot)this.getHibernateTemplate().get(CrPerPot.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPot> findByProperties(CrPerPot pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPot as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPf01aq01())){
                queryString.append(" and model.pf01aq01 = ? ");
                binds.add(pojo.getPf01aq01());
            }
            if(isNotBlankOrNull(pojo.getPf01aj01())){
                queryString.append(" and model.pf01aj01 = ? ");
                binds.add(pojo.getPf01aj01());
            }
            if(isNotBlankOrNull(pojo.getPf01ar01())){
                queryString.append(" and model.pf01ar01 = ? ");
                binds.add(pojo.getPf01ar01());
            }
            if(isNotBlankOrNull(pojo.getPf01zs01())){
                queryString.append(" and model.pf01zs01 = ? ");
                binds.add(pojo.getPf01zs01());
            }
            ...
            return (List<CrPerPot>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPot> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPot>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

...
@Repository
public class CrPerPe01zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPe01zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPe01zh pojo) {
    ...
    public void save(CrPerPe01zh pojo) {
    ...
    public void delete(CrPerPe01zh pojo) {
    ...
    public CrPerPe01zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPe01zh)this.getHibernateTemplate().get(CrPerPe01zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPe01zh> findByProperties(CrPerPe01zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPe01zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPe01zd01())){
                queryString.append(" and model.pe01zd01 = ? ");
                binds.add(pojo.getPe01zd01());
            }
            if(isNotBlankOrNull(pojo.getPe01zq01())){
                queryString.append(" and model.pe01zq01 = ? ");
                binds.add(pojo.getPe01zq01());
            }
            if(isNotBlankOrNull(pojo.getPe01zr01())){
                queryString.append(" and model.pe01zr01 = ? ");
                binds.add(pojo.getPe01zr01());
            }
            ...
            return (List<CrPerPe01zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPe01zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPe01zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

...
@Repository
public class CrPbocD103DAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPbocD103DAO.class);
    ...
    public CrPbocD103DAO() {
    	super();
    	
    }
    
    public void update(CrPbocD103 pojo) {
    ...
    public void save(CrPbocD103 pojo){
       /* this.getHibernateTemplate().setCacheQueries(false);
        if (logger.isDebugEnabled()) {
            logger.info("insert(CrPbocD103) - start"); //$NON-NLS-1$
        }

        try {
            this.getHibernateTemplate().save(pojo);
        } catch (Exception e) {
            logger.error("insert(CrPbocD103)", e); //$NON-NLS-1$

            ExceptionUtil.throwCommonException(e.getMessage(), ErrorCode.ERROR_CODE_BCTL_INSERT, e);
        }

        if (logger.isDebugEnabled()) {
            logger.info("insert(CrPbocD103) - end"); //$NON-NLS-1$
        }*/
    ...
    public void delete(CrPbocD103 pojo) {
    ...
    public CrPbocD103 findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPbocD103)this.getHibernateTemplate().get(CrPbocD103.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPbocD103> findByProperties(CrPbocD103 pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPbocD103 as model where 0= 0 ");
            ...
            return (List<CrPbocD103>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {

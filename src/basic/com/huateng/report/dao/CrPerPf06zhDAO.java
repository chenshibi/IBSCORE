...
@Repository
public class CrPerPf06zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf06zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf06zh pojo) {
    ...
    public void save(CrPerPf06zh pojo) {
    ...
    public void delete(CrPerPf06zh pojo) {
    ...
    public CrPerPf06zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf06zh)this.getHibernateTemplate().get(CrPerPf06zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf06zh> findByProperties(CrPerPf06zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf06zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf06zd01())){
                queryString.append(" and model.pf06zd01 = ? ");
                binds.add(pojo.getPf06zd01());
            }
            if(isNotBlankOrNull(pojo.getPf06zq01())){
                queryString.append(" and model.pf06zq01 = ? ");
                binds.add(pojo.getPf06zq01());
            }
            if(isNotBlankOrNull(pojo.getPf06zr01())){
                queryString.append(" and model.pf06zr01 = ? ");
                binds.add(pojo.getPf06zr01());
            }
            ...
            return (List<CrPerPf06zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf06zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf06zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

...
@Repository
public class CrPerPrmDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPrmDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPrm pojo) {
    ...
    public void save(CrPerPrm pojo) {
    ...
    public void delete(CrPerPrm pojo) {
    ...
    public CrPerPrm findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPrm)this.getHibernateTemplate().get(CrPerPrm.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPrm> findByProperties(CrPerPrm pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPrm as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPb030d01())){
                queryString.append(" and model.pb030d01 = ? ");
                binds.add(pojo.getPb030d01());
            }
            if(isNotBlankOrNull(pojo.getPb030q01())){
                queryString.append(" and model.pb030q01 = ? ");
                binds.add(pojo.getPb030q01());
            }
            if(isNotBlankOrNull(pojo.getPb030q02())){
                queryString.append(" and model.pb030q02 = ? ");
                binds.add(pojo.getPb030q02());
            }
            if(isNotBlankOrNull(pojo.getPb030r01())){
                queryString.append(" and model.pb030r01 = ? ");
                binds.add(pojo.getPb030r01());
            }
            ...
            return (List<CrPerPrm>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPrm> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPrm>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

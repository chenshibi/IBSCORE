...
@Repository
public class CrPerPf03zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf03zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf03zh pojo) {
    ...
    public void save(CrPerPf03zh pojo) {
    ...
    public void delete(CrPerPf03zh pojo) {
    ...
    public CrPerPf03zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf03zh)this.getHibernateTemplate().get(CrPerPf03zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf03zh> findByProperties(CrPerPf03zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf03zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf03zd01())){
                queryString.append(" and model.pf03zd01 = ? ");
                binds.add(pojo.getPf03zd01());
            }
            if(isNotBlankOrNull(pojo.getPf03zq01())){
                queryString.append(" and model.pf03zq01 = ? ");
                binds.add(pojo.getPf03zq01());
            }
            if(isNotBlankOrNull(pojo.getPf03zr01())){
                queryString.append(" and model.pf03zr01 = ? ");
                binds.add(pojo.getPf03zr01());
            }
            ...
            return (List<CrPerPf03zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf03zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf03zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

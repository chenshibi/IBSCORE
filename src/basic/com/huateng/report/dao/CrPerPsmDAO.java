...
@Repository
public class CrPerPsmDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPsmDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPsm pojo) {
    ...
    public void save(CrPerPsm pojo) {
    ...
    public void delete(CrPerPsm pojo) {
    ...
    public CrPerPsm findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPsm)this.getHibernateTemplate().get(CrPerPsm.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPsm> findByProperties(CrPerPsm pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPsm as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPc010q01())){
                queryString.append(" and model.pc010q01 = ? ");
                binds.add(pojo.getPc010q01());
            }
            if(isNotBlankOrNull(pojo.getPc010q02())){
                queryString.append(" and model.pc010q02 = ? ");
                binds.add(pojo.getPc010q02());
            }
            if(isNotBlankOrNull(pojo.getPc010s01())){
                queryString.append(" and model.pc010s01 = ? ");
                binds.add(pojo.getPc010s01());
            }
            if(isNotBlankOrNull(pojo.getPc010d01())){
                queryString.append(" and model.pc010d01 = ? ");
                binds.add(pojo.getPc010d01());
            }
            ...
            return (List<CrPerPsm>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPsm> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPsm>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

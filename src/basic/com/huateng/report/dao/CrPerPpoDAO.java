...
@Repository
public class CrPerPpoDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPpoDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPpo pojo) {
    ...
    public void save(CrPerPpo pojo) {
    ...
    public void delete(CrPerPpo pojo) {
    ...
    public CrPerPpo findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPpo)this.getHibernateTemplate().get(CrPerPpo.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPpo> findByProperties(CrPerPpo pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPpo as model where 0= 0 ");
            List<Object> binds = new ArrayList<Object>();
            if(isNotBlankOrNull(pojo.getId())){
                queryString.append(" and model.id = ? ");
                binds.add(pojo.getId());
            }
            if(isNotBlankOrNull(pojo.getPc040s01())){
                queryString.append(" and model.pc040s01 = ? ");
                binds.add(pojo.getPc040s01());
            }
            ...
            return (List<CrPerPpo>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPpo> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPpo>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

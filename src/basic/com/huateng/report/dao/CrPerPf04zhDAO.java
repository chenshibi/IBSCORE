...
@Repository
public class CrPerPf04zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf04zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf04zh pojo) {
    ...
    public void save(CrPerPf04zh pojo) {
    ...
    public void delete(CrPerPf04zh pojo) {
    ...
    public CrPerPf04zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf04zh)this.getHibernateTemplate().get(CrPerPf04zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf04zh> findByProperties(CrPerPf04zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf04zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf04zd01())){
                queryString.append(" and model.pf04zd01 = ? ");
                binds.add(pojo.getPf04zd01());
            }
            if(isNotBlankOrNull(pojo.getPf04zq01())){
                queryString.append(" and model.pf04zq01 = ? ");
                binds.add(pojo.getPf04zq01());
            }
            if(isNotBlankOrNull(pojo.getPf04zr01())){
                queryString.append(" and model.pf04zr01 = ? ");
                binds.add(pojo.getPf04zr01());
            }
            ...
            return (List<CrPerPf04zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf04zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf04zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

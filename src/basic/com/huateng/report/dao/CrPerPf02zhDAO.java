...
@Repository
public class CrPerPf02zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf02zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf02zh pojo) {
    ...
    public void save(CrPerPf02zh pojo) {
    ...
    public void delete(CrPerPf02zh pojo) {
    ...
    public CrPerPf02zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf02zh)this.getHibernateTemplate().get(CrPerPf02zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf02zh> findByProperties(CrPerPf02zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf02zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf02zd01())){
                queryString.append(" and model.pf02zd01 = ? ");
                binds.add(pojo.getPf02zd01());
            }
            if(isNotBlankOrNull(pojo.getPf02zq01())){
                queryString.append(" and model.pf02zq01 = ? ");
                binds.add(pojo.getPf02zq01());
            }
            if(isNotBlankOrNull(pojo.getPf02zr01())){
                queryString.append(" and model.pf02zr01 = ? ");
                binds.add(pojo.getPf02zr01());
            }
            ...
            return (List<CrPerPf02zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf02zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf02zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }

...
@Repository
public class CrPerPf01zhDAO extends HibernateDaoSupport {

    private static final Logger log = Logger.getLogger(CrPerPf01zhDAO.class);
    @Resource(name = "mySessionFactory")
    public void setSuperSessionFactory(SessionFactory sessionFactory) {
        super.setSessionFactory(sessionFactory);
    }
    public void update(CrPerPf01zh pojo) {
    ...
    public void save(CrPerPf01zh pojo) {
    ...
    public void delete(CrPerPf01zh pojo) {
    ...
    public CrPerPf01zh findById(java.lang.String id) {
        log.info("getting pojo instance with id: " + id);
        try {
            return (CrPerPf01zh)this.getHibernateTemplate().get(CrPerPf01zh.class, id);
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf01zh> findByProperties(CrPerPf01zh pojo) {
        log.info("finding pojo instance with properties: " + pojo.toString());
        try {
            StringBuffer queryString = new StringBuffer();
            queryString.append("from CrPerPf01zh as model where 0= 0 ");
            ...
            if(isNotBlankOrNull(pojo.getPf01zd01())){
                queryString.append(" and model.pf01zd01 = ? ");
                binds.add(pojo.getPf01zd01());
            }
            if(isNotBlankOrNull(pojo.getPf01zq01())){
                queryString.append(" and model.pf01zq01 = ? ");
                binds.add(pojo.getPf01zq01());
            }
            if(isNotBlankOrNull(pojo.getPf01zr01())){
                queryString.append(" and model.pf01zr01 = ? ");
                binds.add(pojo.getPf01zr01());
            }
            ...
            return (List<CrPerPf01zh>)getHibernateTemplate().find(queryString.toString(), binds.toArray());
        } catch (RuntimeException re) {
    ...
    public List<CrPerPf01zh> findByIdProperties(String ids) {
        ...
        try {
        	...
        	return  (List<CrPerPf01zh>)getHibernateTemplate().find(queryString.toString(), null);
        } catch (RuntimeException re) {
            log.error("find by properties failed", re);
            throw re;
        }
